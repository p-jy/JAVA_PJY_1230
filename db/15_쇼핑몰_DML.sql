...
#관리자가 다음의 정보로 회원가입을 했을 때 필요한 쿼리
...
BEGIN
	...
	# PR_CODE가 _CA_CODE로 시작하는 제품의 개수를 구하여 +1
	SET _COUNT = (SELECT COUNT(*) FROM PRODUCT WHERE PR_CODE LIKE CONCAT(_CA_CODE, '%')) + 1;
    # 제품의 개수를 이용하여 3자리 문자열을 만듦. 빈자리는 앞을 0으로 채움
    SET _COUNT_STR = LPAD(_COUNT, 3, "0");
    # 제품 코드는 _CA_CODE와 위에서 구한 3자리 문자열을 합하여 구함.
    SET _PR_CODE = CONCAT(_CA_CODE, _COUNT_STR);
    # _CA_CODE를 이용하여 PR_CA_NUM을 구함
...
UPDATE BUY SET BU_STATE = "구매확정", BU_FINAL_DATE = NOW() WHERE BU_ME_ID = "abc123";