...
# CGV강남을 선택하고, 미키 17을 선택했을 때 상영가능한 날짜를 조회하는 쿼리
SELECT DISTINCT SD_DATE FROM `SCHEDULE`
JOIN SCREEN ON SC_NUM = SD_SC_NUM
WHERE SD_MV_NUM = (SELECT MV_NUM FROM MOVIE WHERE MV_TITLE = "미키 17")
AND SC_TT_NUM = (SELECT TT_NUM FROM THEATER WHERE TT_NAME = "CGV강남");
...
# CGV강남 1관의 3월 11일 16:20에 상영하는 미키 17의 좌석을 조회하는 쿼리
SELECT SE_NAME FROM SEAT
...
SELECT * FROM MOVIE
WHERE MV_DATE BETWEEN SUBDATE(NOW(), INTERVAL 1 MONTH) AND NOW();

# 각 영화별 예매수를 조회하는 쿼리
# 예매수는 상영시간이 지나지않은 예매만 해당
SELECT MV_TITLE, COUNT(*) 예매수 FROM TICKET_LIST
JOIN TICKET ON TI_NUM = TL_TI_NUM
JOIN `SCHEDULE` ON SD_NUM = TI_SD_NUM
LEFT JOIN MOVIE ON MV_NUM = SD_MV_NUM
WHERE TI_STATE = "결제" AND SD_DATE >= NOW() AND SD_TIME > TIME(NOW())
GROUP BY MV_NUM;
...
SELECT RANK() OVER(ORDER BY 예매율 DESC) 순위, M.* FROM (
	SELECT MOVIE.*, SUM(TI_TEEN + TI_ADULT) 예매수, (SUM(TI_TEEN + TI_ADULT) * 100 / SUM(SUM(TI_TEEN + TI_ADULT)) OVER()) AS 예매율
	FROM MOVIE
	LEFT JOIN `SCHEDULE` ON SD_MV_NUM = MV_NUM
	LEFT JOIN TICKET ON TI_SD_NUM = SD_NUM
...
SELECT MOVIE.*, SUM(TI_TEEN) `예매수` FROM MOVIE
LEFT JOIN `SCHEDULE` ON MV_NUM = SD_MV_NUM
LEFT JOIN TICKET ON SD_NUM = TI_SD_NUM
WHERE TI_STATE = "결제"
GROUP BY MV_NUM
ORDER BY 예매수 DESC LIMIT 0, 1;