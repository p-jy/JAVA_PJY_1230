...
public class Ex01_Client {
	 ...
	 * 	  - ex) ABC001, 문구, 볼펜, 빨강, 1000 / DEF001, 의류, 셔츠, XL, 30000 / XYZ001, 식품, 우유1L, 딸기, 2000 (XYZ002, 식품, 딸기우유, 1L, 2000)
	 * 	  - 각 분류마다 분류코드가 지정
	 ...
	 * 	- 분류를 이용하여 조회
	 * 	  - 문구, 의류, 식품, 가전, 기타, 전체
	 * 	- 제품 코드, 제품명, 오션, 수량, 가격이 조회
	 * - 회원가입
	 * 	- 아이디, 비번, 비번확인을 입력해서 회원 가입
	 * - 로그인
	 * 	- 아이디, 비번을 입력하여 회원이면 제품 조회, 아니면 메인으로 돌아감
	 * 	- 관리자이면 관리자 메뉴로 이동
	 * 
	 */
	...
	static private List<Product> list = new ArrayList<Product>();
...
//		list.add(new ClothingProduct());
		...
		members.add(new Member("admin", "admin", "관리자")); //관리자 샘플 데이터
	...
	private static void login() {
		...
		if(index < 0) {
			...
			return;
		} else if(!members.get(index).getPw().equals(pw)) {
