...
<body>
	<h1>게시글 수정</h1>
	<form action="<c:url value="/post/update/${post.po_num}"/>" method="post">
		...
		<div class="form-group mt-3">
			...
			<select class="form-control" id="board" name="po_bo_num">
				<c:forEach items="${list}" var="board">
					...
					<option value="${board.bo_num}" <c:if test="${board.bo_num == post.po+bo_num}">selected</c:if>>${board.bo_name}</option>
				</c:forEach>
			</select>
		</div>
		<div class="form-group mt-3">
